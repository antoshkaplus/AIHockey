...
public class AIManager {
    ...
    public static final double ANGLE_CONSIDERED_DEVIATION = DEGREE;
    ...
    AIRectangle hisCentralZone;
    AIRectangle myCentralZone;
...
//    }
    ...
    boolean canAngleScore(AILine net, AIPoint source, double angle) {
        ...
        return abs(angleOne) >= ANGLE_CONSIDERED_DEVIATION && abs(angleTwo) >= ANGLE_CONSIDERED_DEVIATION &&
               angleOne * angleTwo < 0;
    }
    ...
    boolean canGoalieIntercept(AIHockeyist goalie,
        ...
        if (pointGoalieDistance < PUCK_RADIUS + HOCKEYIST_RADIUS ||
    ...
    public class AIHockeyist extends AIUnit {
        ...
        public AIPoint puckStrikeLocation() {
            AIPoint p = AI.unitVector(getAngle());
            p.scale(PUCK_BINDING_RANGE);
    ...
    private class SwingStrikeScore {
        ...
        AIMove move(AIHockeyist hockeyist) {
            ...
            if (myZone.isInside(hockeyist.getLocation()) &&
                    turningLocation.distance(hockeyist.getLocation()) > LOCATION_RADIUS) {
                ...
                r.scale(d);
                p.translate(r);
                if (p.distance(turningLocation) < LOCATION_RADIUS) {
            ...
            if (hockeyist.distanceTo(turningLocation) < LOCATION_RADIUS) {
                move.setTurn(turnAngle);
                if (abs(turnAngle) < DEGREE &&
                        (hockeyist.getSpeed().scalar() < 1 || speedTurnAngle < DEGREE)) {
                    move.setSpeedUp(1);
                }
                return move;
            }
            ...
            if (hisZone.isInside(hockeyist.getLocation()) &&
                    !canAngleScore(hisNetSegment, hockeyist.getLocation(), hockeyist.getAngle())) {
                move.setValid(false);
                return move;
            }
            ...
            AIFriction friction = AIFriction.getInstance();
            if (friction.hockeyistAfterTicks(SWING_TICKS + 10, hockeyist.getSpeedScalar()).distance > dist) {
                // time to swing
            ...
            if (hockeyist == puckOwner) {
                if (hockeyist.distanceTo(rink.origin) < 3 * HOCKEYIST_RADIUS) {
                    if (hockeyist.getSpeedScalar() < 0.04) {
                        ...
                        if (abs(angle) < 0.01) {
                            //move.setAction(ActionType.STRIKE);
                            if (hockeyist.getLastAction() == ActionType.SWING && currentTick - hockeyist.getLastActionTick() > 11) {
                                move.setAction(ActionType.STRIKE);
                            } else {
                                move.setAction(ActionType.SWING);
                            }
